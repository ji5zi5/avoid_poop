...
export const multiplayerHazardSnapshotSchema = z.object({
  ...
  height: z.number().positive(),
  speed: z.number(),
  variant: z.enum(['small', 'medium', 'large', 'boss', 'giant']),
  behavior: z.enum(['none', 'split', 'bounce']).optional(),
  velocityX: z.number().optional(),
  gravity: z.number().optional(),
  splitAtY: z.number().optional(),
  splitChildCount: z.number().int().positive().optional(),
  splitChildSize: z.number().positive().optional(),
  splitChildSpeed: z.number().positive().optional(),
  splitChildSpread: z.number().positive().optional(),
  bouncesRemaining: z.number().int().nonnegative().optional(),
...
export const multiplayerGameSnapshotSchema = z.object({
  ...
  bossEncounterDuration: z.number().nonnegative(),
  bossThemeId: z.string().min(1).nullable(),
  bossThemeLabel: z.string(),
  bossPatternQueue: z.array(z.string().min(1)),
  bossPatternIndex: z.number().int().nonnegative(),
  bossPatternActiveId: z.string().min(1).nullable(),
  bossPatternPhase: z.enum(['idle', 'telegraph', 'attack', 'cooldown']),
  bossTelegraphText: z.string(),
  bossTelegraphTimer: z.number().nonnegative(),
  options: roomOptionsSchema,
